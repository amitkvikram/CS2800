...
void merge(int l, int r, int end){
	FILE *fp1 = fopen(FileName, "r+");				//Opening Input File.
	FILE *fp2 = fopen(FileName, "r+");				//Opening Input File.
>>>>>>> 9fcaf1abc59fc8f9b9494d541d15488ffd50781d
	FILE *f_temp = fopen("temp.bin","r+");			//Opening file for storing the merged elements
	FILE *fp1 = fopen("input.bin","r+");
	FILE *fp2 = fopen("input.bin","r+");
	int i = l, j = r;
	...
	fseek(f_temp, 0*sizeof(long), SEEK_SET);
	fseek(fp1, l*sizeof(long), SEEK_SET);
...
||||||| merged common ancestors
			merge( i, min(i+blk_sz, sz), min(i+2*blk_sz, sz));	//calling function to merge
=======
			printf("block size =  %d\n",blk_sz);
			merge( i, min(i+blk_sz, sz), min(i+2*blk_sz, sz));	//calling function to merge
>>>>>>> 9fcaf1abc59fc8f9b9494d541d15488ffd50781d
